...
fun buildTalkDeck(shortTrack: Boolean): List<DeckSlide> {
    val all = listOf(
        ...
        DeckSlide(
            ...
            notes = "Show on request. zIndex and the ellipse angle are the heart of it. graphicsLayer is depth, not placement.",
            kind = SlideKind.Code(
                ...
                lines = listOf(
                    ...
                    "        val rx = min(w, h) * 0.38f * expand",
                    "        val ry = rx * 0.38f",
                    "        placeables.forEachIndexed { i, p ->",
                    "            val a = PI/2 + 2*PI * (i - selected) / n",
                    "            p.placeRelative(",
                    "                x = (cx + rx * cos(a) - p.width/2).toInt(),",
                    "                y = (cy + ry * sin(a) - p.height/2).toInt(),",
                    "                zIndex = frontness(i),  // selected at 6 o'clock",
                    "            )",
                    "        }",
                    "    }",
                    "}",
                ),
                accentLines = setOf(8, 12),
            ),
        ),
        DeckSlide(
            id = "excess",
